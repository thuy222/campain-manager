...
1. **Scope over-reach on `recipients.md`.** Claude proposed implementing the full recipients feature (list / replace / demo-mark-opened endpoints + UI) even though none of those endpoints were part of the assignment's required surface. I pushed back; we scoped it out and left the spec as a deferred document. Lesson: a ready spec doesn't by itself authorize work — the assignment does.
